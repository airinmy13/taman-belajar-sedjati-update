...
<body>
    ...
    <div class="container">
        ...
        <div class="header">
            ...
            @php
                $createRoute = session('teacher_id') ? route('teacher.questions.create', $game->id) : route('admin.questions.create', $game->id);
            @endphp
            <a href="{{ $createRoute }}" class="btn btn-primary">+ Tambah Soal</a>
        </div>
                        ...
                        @foreach($questions as $index => $question)
                            <tr>
                                ...
                                <td>
                                    <div class="actions">
                                        @php
                                            $editRoute = session('teacher_id') ? route('teacher.questions.edit', $question->id) : route('admin.questions.edit', $question->id);
                                            $deleteRoute = session('teacher_id') ? route('teacher.questions.delete', $question->id) : route('admin.questions.delete', $question->id);
                                        @endphp
                                        <a href="{{ $editRoute }}" class="btn btn-warning btn-sm">Edit</a>
                                        <form action="{{ $deleteRoute }}" method="POST" style="display: inline;" onsubmit="return confirm('Yakin ingin menghapus soal ini?')">
                                            @csrf
                                            @method('DELETE')

...
class GameController extends Controller
{
    ...
    public function index()
    {
        ...
        $gamesQuery = Game::with('teacher')->where('is_active', true);

        // Filter by class level if student has a class
        if ($studentClass) {
            $gamesQuery->where(function($q) use ($studentClass) {
        ...
        $games = $gamesQuery->orderBy('order', 'asc')->get();
    ...
    public function getQuestion($sessionId)
    {
        ...
        // Check if game is custom code
        if ($session->game->game_type == 'custom_code') {
